...
public class EmailService : IEmailService
{
        ...
        Guid vaccineId)
    {
        ...
        if (vaccine == null) throw new ArgumentException($"Vaccine with ID {vaccineId} does not exist.");

        var totalPrice = appointment.AppointmentsVaccines.FirstOrDefault(av => av.VaccineId == vaccineId)?.TotalPrice ??
                         0;
    ...
    <div style='font-family: Arial, sans-serif; line-height: 1.8; color: #333;'>
        ...
        <p style='font-size: 16px; line-height: 1.6;'>We are pleased to inform you that your appointment for the vaccination of <strong>{vaccine.VaccineName ?? "Unknown Vaccine"}</strong> has been successfully scheduled. Please find the details of the appointment below:</p>
        <div style='margin-top: 20px; background-color: #f8f8f8; padding: 20px; border-radius: 8px;'>
            <ul style='font-size: 16px; line-height: 1.6; padding-left: 20px;'>
                ...
                <li><strong style='color: #1e1b4b;'>Dose:</strong> {appointment.AppointmentsVaccines.First().DoseNumber}/{vaccine.RequiredDoses}</li>
                <li><strong style='color: #1e1b4b;'>Total Price:</strong> {totalPriceFormatted}</li>
            </ul>
        </div>
