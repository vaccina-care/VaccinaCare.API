...
namespace VaccinaCare.Application.Service
{
    public class AppointmentService : IAppointmentService
    {
        ...
        /// </summary>
        public async Task<List<AppointmentDTO>> GenerateAppointmentsForSingleVaccine(CreateAppointmentDto request, Guid parentId)
        {
            ...
            foreach (var vaccineId in request.VaccineIds)
            {
                ...
                int nextDose = await _vaccineService.GetNextDoseNumber(request.ChildId, vaccineId);
                if (nextDose > vaccine.RequiredDoses)
                    throw new Exception($"Trẻ đã tiêm đủ số mũi của vaccine {vaccine.VaccineName}.");

                DateTime appointmentDate = request.StartDate;
                ...
                for (int dose = nextDose; dose <= vaccine.RequiredDoses; dose++)
                {
                    ...
                    if (!await _vaccineService.CheckVaccineCompatibility(vaccineId, bookedVaccineIds, appointmentDate))
                        throw new Exception($"Vaccine {vaccine.VaccineName} không thể tiêm cùng các loại vaccine đã đặt trước.");
                        ...
                        AppointmentsVaccines = new List<AppointmentsVaccine>
                {
                    ...
                    await _vaccineRecordService.AddVaccinationRecordAsync(request.ChildId, vaccineId, appointmentDate, dose);
                }
            }
