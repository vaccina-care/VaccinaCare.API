...
namespace VaccinaCare.Application.Service
{
    public class FeedbackService : IFeedbackService
    {
        ...
        public async Task<Pagination<FeedbackDTO>> GetAllFeedbacksAsync(PaginationParameter pagination)
        {
            try
            {
                _logger.Info($"Fetching feedback with pagination: Page {pagination.PageIndex}, Size {pagination.PageSize} ");

                var query = _unitOfWork.FeedbackRepository.GetQueryable();

                int totalFeedbacks = await query.CountAsync();
                ...
                if (!feedbacks.Any())
                {
                    ...
                    return new Pagination<FeedbackDTO>(new List<FeedbackDTO>(), 0, pagination.PageIndex, pagination.PageSize);
                }
